...
{
    return [[[DVDSubpictureAttributes alloc] initWithData:data] autorelease];
}

- (id) initWithData:(NSData*)data
{
    NSAssert(data && [data length] == sizeof(vmgm_subp_attr_t), @"wtf?");
    if (self = [super init]) {
        const vmgm_subp_attr_t* vmgm_subp_attr = [data bytes];
...
- (NSData*) saveAsData:(NSError**)_error
{
    ...
    bzero(&vmgm_subp_attr, sizeof(vmgm_subp_attr_t));
    ...
    return [NSData dataWithBytes:&vmgm_subp_attr length:sizeof(vmgm_subp_attr_t)];
}
