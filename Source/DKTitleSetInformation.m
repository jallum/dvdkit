...
- (id) initWithData:(NSData*)data index:(uint16_t)_index
{
    ...
    if (self = [super init]) {
        ...
        if (vts_ptt_srpt && (vts_ptt_srpt <= vtsi_last_sector)) {
            ...
            for (int i = 0; i < nr_of_srpts; i++, p += 4) {
                ...
                for (const uint8_t* x = bp + start_byte, *lx = x + length; x < lx; x += 4) {
                    [ptts addObject:[DKPartOfTitle partOfTitleWithProgramChain:OSReadBigInt16(x, 0) program:OSReadBigInt16(x, 2)]];
                }
        ...
        if (vtsm_c_adt && (vtsm_c_adt <= vtsi_last_sector)) {
            ...
            while (p < lp) {
                [menuCellAddressTable addObject:[DKCellAddress cellAddressWithData:[data subdataWithRange:NSMakeRange(p - bytes, 12)]]];
                p += 12;
            }
        }
        ...
        if (vts_c_adt && (vts_c_adt <= vtsi_last_sector)) {
            ...
            while (p < lp) {
                [cellAddressTable addObject:[DKCellAddress cellAddressWithData:[data subdataWithRange:NSMakeRange(p - bytes, 12)]]];
                p += 12;
            }
        }
